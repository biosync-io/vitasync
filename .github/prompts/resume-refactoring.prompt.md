...
find apps/api/src/services -name "*.ts" | head -20
find apps/worker/src -name "*.ts" | head -20
find apps/web/src/app/dashboard -name "page.tsx" | head -20
```
...
## Step 5: Verify page integrity (if resuming Phase 6+)

If the previous session touched frontend pages, verify they weren't gutted:

```bash
# Compare page line counts against inventory
cat /tmp/page-inventory.txt 2>/dev/null || \
  find apps/web/src/app -name "page.tsx" -exec sh -c 'echo "$(wc -l < "$1") $1"' _ {} \; | sort -rn
```

Pages that lost >10% of their lines need investigation and restoration.

## Rules

- Do NOT ask for approval. Keep going autonomously.
- Do NOT stop between phases.
- ⛔ Do NOT gut pages — preserve all existing functionality.
