  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        subtitle="Track inbound webhook events from health data providers (Whoop, Fitbit, Garmin, Strava, Withings)."
        actions={
          <div className="flex items-center gap-2 rounded-xl border border-gray-200 dark:border-white/[0.06] bg-white/80 dark:bg-white/[0.03] backdrop-blur-xl px-3 py-2 shadow-sm">
          ...
          <CardContent>
            ...
            <div className="flex flex-wrap gap-1 mt-1.5">
              {Object.keys(providerCounts).length > 0 ? Object.entries(providerCounts).map(([p, c]) => {
                ...
                return <Badge key={p} size="sm" className={`${pc.bg} ${pc.text}`}>{p} <span className="font-bold">{c}</span></Badge>
          ...
          <CardContent>
            ...
            <div className="flex flex-wrap gap-1 mt-1.5">
              {Object.keys(statusCounts).length > 0 ? Object.entries(statusCounts).map(([s, c]) => {
                ...
                return <Badge key={s} variant={s === "processed" ? "success" : s === "rejected" || s === "error" ? "danger" : "default"} dot size="sm">{ss.label} <span className="font-bold">{c}</span></Badge>
              }) : <span className="text-sm text-gray-400 dark:text-gray-500">—</span>}
            </div>
          </CardContent>
        </Card>
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={Webhook}
          title={total === 0 ? "No inbound webhook events recorded yet" : "No events match the current filters"}
          description={total === 0 ? "Events will appear here when providers send webhooks." : "Try adjusting your filters."}
        />
      ) : (
        <>
          {/* Desktop table */}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Badge size="sm" className={`${provColor.bg} ${provColor.text}`}>
                          {log.providerId}
                        </Badge>
                        <Badge
                          variant={log.status === "processed" ? "success" : log.status === "rejected" || log.status === "error" ? "danger" : "default"}
                          dot
                          size="sm"
                        >
                          {st.label}
