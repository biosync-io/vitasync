    ...
    <div>
      ...
      {showCreate && (
        <div className="mb-6 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-gray-900 dark:text-gray-100">New Webhook</h2>
          {error && <p className="mb-3 text-sm text-red-600">{error}</p>}
          <div className="space-y-3">
            <div>
              <label htmlFor="webhook-url" className="block text-xs font-medium text-gray-700 dark:text-gray-300">
                Endpoint URL *
              </label>
              <input
                id="webhook-url"
            ...
            <div>
              <label htmlFor="webhook-secret" className="block text-xs font-medium text-gray-700 dark:text-gray-300">
                Secret (min 16 chars) *
              </label>
              <input
                id="webhook-secret"
            ...
            <div>
              <p className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Events *</p>
            ...
            <div>
              <label htmlFor="webhook-desc" className="block text-xs font-medium text-gray-700 dark:text-gray-300">
                Description
              </label>
              <input
                id="webhook-desc"
      ...
      {isLoading ? (
        ...
        <>
          <div className="space-y-3">
            {pagedHooks.map((hook) => (
            <div key={hook.id} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 shadow-sm">
                ...
                <div className="ml-4 flex items-center gap-2 flex-shrink-0">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                      hook.isActive ? "bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400"
                    }`}
                  >
