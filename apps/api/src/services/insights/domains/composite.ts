...
export const compositeRunners: Record<string, AlgorithmRunner> = {
  ...
  "weekend-weekday-activity": (alg, ctx) => {
    ...
    const dates = ctx
      .dayStats("steps")
      .map((d) => ({ val: d.value, dow: new Date(d.date).getDay() }))
  ...
  "morning-readiness": (alg, ctx) => {
    const sleepV = ctx.vals("sleep")
    const readiness = ctx.vals("readiness_score")
  ...
  "overtraining-risk": (alg, ctx) => {
    ...
    const weekWorkouts = ctx.workouts.filter(
  ...
  "detraining-risk": (alg, ctx) => {
    ...
    const sorted = [...ctx.workouts].sort(
    ...
    const twoWeekCount = ctx.workouts.filter(
  ...
  "sleep-workout-timing": (alg, ctx) => {
    ...
    const workoutDates = new Set(
      ctx.workouts.map((w) => new Date(w.startedAt).toISOString().slice(0, 10)),
    )
  ...
  "hydration-proxy": (alg, ctx) => {
    ...
    const baseRHR =
      rhr.slice(0, Math.min(14, rhr.length)).reduce((a, b) => a + b, 0) / Math.min(14, rhr.length)
