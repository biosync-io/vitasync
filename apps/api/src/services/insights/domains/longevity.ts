...
export const longevityRunners: Record<string, AlgorithmRunner> = {
  "biological-age": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Estimated biological age: ${bioAge} years. ${offset < -2 ? "Your biomarkers suggest you're aging slower than average — excellent lifestyle habits." : offset < 3 ? "Average aging trajectory. Optimize sleep and activity for improvement." : "Accelerated aging signals detected — prioritize recovery and cardiovascular exercise."}`,
      bioAge,
      "years",
      { low: 25, high: 50 },
      {
        rhrPenalty: +rhrPenalty.toFixed(1),
        hrvBonus: +hrvBonus.toFixed(1),
        sleepScore: +sleepScore.toFixed(1),
        activityBonus: +activityBonus.toFixed(1),
      },
    )
  },
  ...
  "longevity-score": (alg, ctx) => {
    ...
    if (avgRHR < 60) score += 15
    else if (avgRHR < 70) score += 8
    else if (avgRHR > 80) score -= 10
    if (hrv.length >= 7) {
      const avgHRV = hrv.slice(-7).reduce((a, b) => a + b, 0) / Math.min(7, hrv.length)
      if (avgHRV > 50) score += 12
      else if (avgHRV > 30) score += 5
    }
    const avgSleep = sleep.slice(-7).reduce((a, b) => a + b, 0) / Math.min(7, sleep.length)
    if (avgSleep >= 420 && avgSleep <= 540) score += 12
    else if (avgSleep >= 360) score += 5
    else score -= 8
    const avgSteps = steps.slice(-7).reduce((a, b) => a + b, 0) / Math.min(7, steps.length)
    if (avgSteps >= 10000) score += 12
    else if (avgSteps >= 7500) score += 8
    else if (avgSteps < 3000) score -= 10
    if (spo2.length >= 3) {
      const avgSpo2 = spo2.slice(-7).reduce((a, b) => a + b, 0) / Math.min(7, spo2.length)
      if (avgSpo2 >= 97) score += 5
      else if (avgSpo2 < 94) score -= 8
    }
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Longevity index: ${score}/100. ${score >= 75 ? "Outstanding — your health metrics align with longevity research markers." : score >= 50 ? "Good foundation. Focus on the lowest-scoring areas for significant gains." : "Multiple risk factors detected. Small improvements in RHR, sleep, and activity can shift your trajectory."}`,
      score,
      "/100",
      { low: 40, high: 80 },
      {
  ...
  "aging-velocity": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Aging velocity: ${velocity > 0 ? "accelerating" : "decelerating"} (${velocity.toFixed(2)}/day). ${velocity < -0.5 ? "Your biomarkers are improving — biological clock is slowing." : velocity < 0.5 ? "Stable trajectory — maintain current habits." : "Biomarker trends suggest accelerated wear. Prioritize recovery."}`,
      Number(velocity.toFixed(2)),
      "rate",
      { low: -1, high: 1 },
      { rhrSlope: +rhrSlope.toFixed(4), hrvSlope: +hrvSlope.toFixed(4) },
    )
  },
  ...
  "cardiovascular-reserve": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Cardiovascular reserve: ${Math.round(reserve)} bpm (peak ${peakHR} - resting ${Math.round(avgRHR)}). ${reserve > 100 ? "Excellent cardiac capacity — strong longevity marker." : reserve > 70 ? "Adequate reserve. Interval training can improve this." : "Limited cardiac reserve — may benefit from progressive aerobic training."}`,
      Math.round(reserve),
      "bpm",
      { low: 70, high: 120 },
      { peakHR, avgRHR: Math.round(avgRHR) },
    )
  },
  ...
  "hrv-age-gap": (alg, ctx) => {
    ...
    const sev: InsightSeverity =
      yearsYounger > 5 ? "positive" : yearsYounger > -5 ? "info" : "warning"
    return ctx.makeInsight(
      alg,
      sev,
      `HRV age gap: ${yearsYounger > 0 ? "+" : ""}${yearsYounger} years ${yearsYounger > 0 ? "younger" : "older"} than baseline. Average HRV: ${avgHRV.toFixed(1)}ms. ${yearsYounger > 5 ? "Exceptional autonomic health." : yearsYounger > -5 ? "Typical HRV for your activity level." : "Low HRV — stress management and aerobic exercise can improve this."}`,
      yearsYounger,
      "years",
      { low: -10, high: 15 },
      { avgHRV: +avgHRV.toFixed(1), expectedHRV },
    )
  },
  ...
  "sleep-longevity-alignment": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Sleep-longevity alignment: ${alignment}%. Average: ${avgHrs.toFixed(1)}h/night. ${alignment >= 80 ? "Perfectly in the 7-8h longevity sweet spot." : alignment >= 50 ? "Close to optimal. Research shows 7-8h is the longevity goldilocks zone." : "Outside optimal range. Both too little and too much sleep correlate with reduced lifespan."}`,
      alignment,
      "%",
      { low: 50, high: 90 },
      { avgHrs: +avgHrs.toFixed(1), optimalRange: "7-8h" },
    )
  },

  "resting-metabolic-health": (alg, ctx) => {
    const rhr = ctx.vals("resting_heart_rate")
    const bp = ctx.vals("blood_pressure")
    ...
    if (bp.length >= 3) {
      const avg = bp.slice(-3).reduce((a, b) => a + b, 0) / 3
      total++
      if (avg < 130) signals++
    }
    if (glucose.length >= 3) {
      const avg = glucose.slice(-3).reduce((a, b) => a + b, 0) / 3
      total++
      if (avg < 100) signals++
    }
    if (weight.length >= 7) {
      const cv = coefficientOfVariation(weight.slice(-14))
      total++
      if (cv < 2) signals++
    }
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Metabolic health markers: ${signals}/${total} optimal. ${pct >= 75 ? "Excellent metabolic health — reduces chronic disease risk significantly." : pct >= 50 ? "Mixed metabolic signals. Target the weakest markers." : "Multiple metabolic risk factors flagged."}`,
      pct,
      "%",
      { low: 50, high: 100 },
      { signals, total },
  ...
  "inflammation-proxy": (alg, ctx) => {
    ...
    if (temp.length >= 3) {
      const avgTemp = temp.slice(-3).reduce((a, b) => a + b, 0) / 3
      if (avgTemp > 37.2) score += 35
    }
    const sev: InsightSeverity = score < 20 ? "positive" : score < 50 ? "info" : "warning"
    return ctx.makeInsight(
      alg,
      sev,
      `Systemic inflammation proxy: ${score}/100. ${score < 20 ? "Low inflammation markers — excellent for long-term health." : score < 50 ? "Mild elevation in inflammatory biomarkers." : "Elevated inflammation signals: RHR up, HRV down" + (temp.length >= 3 ? ", temperature elevated" : "") + ". Rest and anti-inflammatory nutrition recommended."}`,
      score,
      "/100",
      { low: 0, high: 50 },
      { rhrDelta: +(rhrRecent - rhrBase).toFixed(1), hrvRatio: +(hrvRecent / hrvBase).toFixed(2) },
    )
  },
  ...
  "fitness-age": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Estimated fitness age: ${age} years. ${age < 30 ? "Elite fitness level — consistent with athletes 10+ years younger." : age < 40 ? "Good functional fitness. Increase workout intensity for further gains." : "Room for improvement — progressive exercise can reduce fitness age significantly."}`,
      age,
      "years",
      { low: 20, high: 45 },
      {
        avgSteps: Math.round(avgSteps),
        avgRHR: Math.round(avgRHR),
        weeklyWorkouts: +weeklyWorkouts.toFixed(1),
      },
    )
  },
}
