...
export const womensHealthRunners: Record<string, AlgorithmRunner> = {
  "cycle-phase-detection": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Estimated cycle phase: ${phase} (${confidence}% confidence). ${phase === "luteal" ? "Higher RHR and lower HRV typical in luteal phase. You may need more recovery time and sleep." : phase === "follicular" ? "Follicular phase detected — typically the best window for high-intensity training and PRs." : "Transition period — monitor for phase shift in coming days."}`,
      confidence,
      "%",
      { low: 30, high: 80 },
      { phase, rhrDelta: +rhrDelta.toFixed(1), hrvDelta: +hrvDelta.toFixed(1) },
    )
  },
  ...
  "phase-adapted-training": (alg, ctx) => {
    ...
    if (isLuteal) {
      rec =
        "Luteal phase indicators detected. Reduce peak intensity by 5-10%, increase warm-up duration, and prioritize yoga/mobility. Carb intake may need to increase for sustained energy."
      intensity = 65
    } else {
      rec =
        "Follicular/early phase indicators — optimal window for strength gains, HIIT, and setting PRs. Your body recovers faster in this phase."
      intensity = 85
    }
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Phase-adapted training: ${intensity}% intensity recommended. ${rec}`,
      intensity,
      "%",
      { low: 50, high: 90 },
      { rhrDiff: +diff.toFixed(1), phase: isLuteal ? "luteal" : "follicular" },
    )
  },
  ...
  "energy-cycle-pattern": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `${cyclical ? `Cyclical energy pattern detected. Peak energy week ~${peakWeek}, low energy week ~${lowWeek} of your cycle. Plan demanding activities for peak weeks and allow more recovery during low weeks.` : "Energy levels are relatively stable across weeks — no strong cyclical pattern detected."}`,
      Math.round(variance),
      "min variance",
      { low: 10, high: 40 },
      { weeklyAvgs: pattern.map((v) => Math.round(v)), peakWeek, lowWeek },
    )
  },
  ...
  "pms-symptom-predictor": (alg, ctx) => {
    ...
    if (stress.length >= 7) {
      const stressSlope = linearSlope(stress.slice(-7))
      if (stressSlope > 0.5) risk += 20
    }
    if (sleep.length >= 7) {
      const sleepSlope = linearSlope(sleep.slice(-7))
      if (sleepSlope < -3) risk += 15
    }
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `PMS symptom risk: ${risk}%. ${risk < 25 ? "Low likelihood of premenstrual symptoms based on current biomarkers." : risk < 50 ? "Moderate indicators. Consider proactive magnesium supplementation and stress management." : "Biomarkers suggest approaching premenstrual phase — increase hydration, prioritize sleep, and moderate exercise intensity."}`,
      risk,
      "%",
      { low: 0, high: 50 },
      { rhrSlope: +rhrSlope.toFixed(2), hrvSlope: +hrvSlope.toFixed(2) },
    )
  },
  ...
  "iron-deficiency-proxy": (alg, ctx) => {
    ...
    if (spo2.length >= 7) {
      const avg = spo2.slice(-7).reduce((a, b) => a + b, 0) / 7
      if (avg < 96) risk += 25
    }
    if (sleep.length >= 14) {
    ...
    const olderWorkouts = ctx.workouts.filter((w) => {
      const d = new Date(w.startedAt)
      return d < new Date(Date.now() - 14 * 86400000) && d >= new Date(Date.now() - 28 * 86400000)
    })
    if (recentWorkouts.length > 0 && olderWorkouts.length > 0) {
      const recentAvgHR =
        recentWorkouts
          .filter((w) => w.avgHeartRate)
          .reduce((a, w) => a + (w.avgHeartRate ?? 0), 0) / recentWorkouts.length
      const olderAvgHR =
        olderWorkouts.filter((w) => w.avgHeartRate).reduce((a, w) => a + (w.avgHeartRate ?? 0), 0) /
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Iron deficiency risk indicators: ${risk}%. ${risk < 20 ? "No concerning patterns detected." : risk < 45 ? "Some biomarkers warrant monitoring. Consider tracking dietary iron intake." : "Multiple indicators suggest possible iron deficiency — rising RHR, increased fatigue, performance decline. Consider blood work to check ferritin and hemoglobin levels."}`,
      risk,
      "%",
      { low: 0, high: 40 },
      { rhrTrend: +rhrTrend.toFixed(2) },
    )
  },
  ...
  "fertility-window-indicators": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Fertility indicators: ${isPreOvulation ? "Pre-ovulatory phase likely — RHR at cycle minimum." : "Post-ovulatory phase likely — elevated RHR baseline."} Confidence: ${confidence}%. Note: This is an estimate based on biometric patterns, not a medical fertility assessment.`,
      confidence,
      "% confidence",
      { low: 30, high: 80 },
      { isPreOvulation, rhrVariability: +rhrVariability.toFixed(1) },
    )
  },
  ...
  "menstrual-migraine-risk": (alg, ctx) => {
    ...
    const hrvDrop =
      hrv.slice(-3).reduce((a, b) => a + b, 0) / 3 - hrv.slice(-7).reduce((a, b) => a + b, 0) / 7
    ...
    if (sleep.length >= 3) {
      const avg = sleep.slice(-3).reduce((a, b) => a + b, 0) / 3
      if (avg < 360) risk += 15
    }
    risk = Math.min(100, Math.round(risk))
    const sev: InsightSeverity = risk < 25 ? "positive" : risk < 50 ? "info" : "warning"
    return ctx.makeInsight(
      alg,
      sev,
      `Menstrual migraine risk: ${risk}%. ${risk < 25 ? "Low risk — stable biomarkers." : risk < 50 ? "Moderate risk. Stay hydrated and manage screen time." : "Elevated risk from rapid hormonal shifts indicated by RHR/HRV changes. Preemptive magnesium, hydration, and reduced triggers recommended."}`,
      risk,
      "%",
      { low: 0, high: 45 },
      { rhrSwing: +rhrSwing.toFixed(1), hrvDrop: +hrvDrop.toFixed(1) },
    )
  },
  ...
  "perimenopause-indicators": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Perimenopause indicators: ${score}/100. ${score < 25 ? "Biomarker patterns appear regular." : score < 50 ? "Some increased variability in long-term patterns. This may be normal or indicate hormonal transition." : "Pattern irregularity increasing over 2 months. Consistent with perimenopause — consider discussing with your healthcare provider for hormone panel testing."}`,
      score,
      "/100",
      { low: 15, high: 50 },
      { rhrCV: +rhrCV.toFixed(1), hrvCV: +hrvCV.toFixed(1), sleepCV: +sleepCV.toFixed(1) },
    )
  },
}
