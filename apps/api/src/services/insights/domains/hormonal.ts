...
export const hormonalRunners: Record<string, AlgorithmRunner> = {
  "cortisol-proxy": (alg, ctx) => {
    ...
    if (stress.length >= 3) {
      ...
      cortisolEstimate += Math.max(0, (avg - 50) * 0.4)
    }
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Cortisol stress proxy: ${cortisolEstimate}/100 (${cortisolEstimate < 45 ? "low — healthy stress regulation" : cortisolEstimate < 65 ? "moderate — within normal range" : "elevated — consider stress management"}). Based on HRV decline rate, RHR trends, and stress patterns. ${cortisolEstimate >= 65 ? "Chronic elevation impairs immune function, sleep, and body composition." : ""}`,
      cortisolEstimate,
      "/100",
      { low: 20, high: 60 },
      { hrvTrend: +hrvTrend.toFixed(2), rhrTrend: +rhrTrend.toFixed(2) },
    )
  },
  ...
  "testosterone-proxy": (alg, ctx) => {
    ...
    if (hrvAvg > hrvBase * 1.05) score += 10
    else if (hrvAvg < hrvBase * 0.9) score -= 10
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Anabolic environment score: ${score}/100. ${score >= 70 ? "Favorable hormone milieu — strong conditions for muscle growth and recovery." : score >= 45 ? "Moderate anabolic status. Prioritize sleep quality to optimize." : "Low anabolic drive — sleep debt and stress are suppressing recovery hormones."}`,
      score,
      "/100",
      { low: 40, high: 75 },
      { avgSleepMin: Math.round(avgSleep), weeklyWorkouts: recentWorkouts.length },
    )
  },
  ...
  "hormonal-rhythm-stability": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Circadian rhythm stability: ${clamped}/100. ${clamped >= 70 ? "Strong — consistent patterns support healthy hormone cycling." : clamped >= 45 ? "Some variability detected. Aim for consistent sleep/wake times." : "Unstable circadian rhythm. Irregular schedules disrupt cortisol, melatonin, and growth hormone production."}`,
      clamped,
      "/100",
      { low: 40, high: 75 },
      { sleepCV: +sleepCV.toFixed(1), rhrCV: +rhrCV.toFixed(1) },
    )
  },
  ...
  "stress-hormone-accumulation": (alg, ctx) => {
    ...
    const prevWeekAvg =
      stress.slice(-14, -7).length > 0
        ? stress.slice(-14, -7).reduce((a, b) => a + b, 0) / stress.slice(-14, -7).length
        : weekAvg
    const accumulation = Math.round(weekAvg * 0.6 + Math.max(0, weekAvg - prevWeekAvg) * 0.4)
    const chronic = weekAvg > 60 && prevWeekAvg > 55
    const sev: InsightSeverity =
      accumulation < 35 ? "positive" : accumulation < 55 ? "info" : chronic ? "critical" : "warning"
    return ctx.makeInsight(
      alg,
      sev,
      `Stress accumulation index: ${accumulation}. ${chronic ? "CHRONIC: Sustained high stress over 2+ weeks. This pattern is associated with adrenal fatigue, metabolic dysfunction, and reduced immune function." : accumulation < 35 ? "Well-managed stress levels." : "Rising stress load — schedule active recovery."}`,
      accumulation,
      "",
      { low: 20, high: 55 },
      { weekAvg: +weekAvg.toFixed(1), prevWeekAvg: +prevWeekAvg.toFixed(1), chronic },
    )
  },
  ...
  "recovery-hormone-window": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Growth hormone window quality: ${ghWindow}%. Est. ${Math.round(deepSleepProxy)} min deep sleep. ${ghWindow >= 70 ? "Optimal GH secretion conditions — supports tissue repair, fat metabolism, and muscle recovery." : ghWindow >= 45 ? "Moderate GH production. Try sleeping in a cooler room to enhance deep sleep." : "Poor GH window — insufficient deep sleep. Avoid alcohol and screens before bed."}`,
      ghWindow,
      "%",
      { low: 40, high: 80 },
      { estDeepSleepMin: Math.round(deepSleepProxy) },
    )
  },
  ...
  "metabolic-hormone-balance": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Metabolic hormone balance: ${balance}/100. ${balance >= 65 ? "Healthy leptin/ghrelin signaling — appetite regulation should be normal." : balance >= 40 ? "Some appetite dysregulation possible. Prioritize sleep quality." : "Disrupted hunger hormones from poor sleep. You may experience increased cravings and reduced satiety signals."}`,
      balance,
      "/100",
      { low: 35, high: 70 },
      { avgSleepMin: Math.round(avgSleep), rhrTrend: +rhrTrend.toFixed(2) },
    )
  },
  ...
  "thyroid-function-proxy": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Thyroid function proxy: ${score}/100. ${score >= 65 ? "Stable metabolic markers consistent with normal thyroid function." : score >= 40 ? "Some variation in metabolic indicators. Monitor for persistent changes." : "Metabolic patterns suggest possible thyroid imbalance. Consult your healthcare provider if symptoms persist."}`,
      score,
      "/100",
      { low: 40, high: 75 },
      { rhrAvg: +rhrAvg.toFixed(1), rhrDelta: +rhrDelta.toFixed(1) },
    )
  },
  ...
  "adrenal-fatigue-risk": (alg, ctx) => {
    ...
    return ctx.makeInsight(
      alg,
      sev,
      `Adrenal fatigue risk: ${risk}%. ${risk < 20 ? "Low — healthy stress-recovery balance." : risk < 40 ? "Mild risk. Incorporate mindfulness or yoga." : "High risk from sustained stress + declining HRV. Implement strict recovery protocols — rest days, sleep optimization, adaptogenic practices."}`,
      risk,
      "%",
      { low: 0, high: 40 },
      { stressAvg14d: +stressAvg.toFixed(1), hrvSlope14d: +hrvSlope.toFixed(2) },
    )
  },
}
